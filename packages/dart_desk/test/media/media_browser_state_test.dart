...
void main() {
  group('MediaBrowserState', () {
    ...
    test(
      'confirmAndDelete does not call deleteMedia when dialog is canceled',
      () async {
        await state.assetsData.future;
    ...
    test(
      'confirmAndDelete deletes and reloads when confirmed (usage=0)',
      () async {
        await state.assetsData.future;
        await state.confirmAndDelete(
          assetId: 'asset-icon',
    ...
    test(
      'confirmAndDelete passes usageCount from dataSource to confirm callback',
      () async {
        await state.assetsData.future;
        int? seen;
        await state.confirmAndDelete(
